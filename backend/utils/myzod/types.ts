  ...
  parse(
    ...
    if (keys.length === 0 && keySig) {
      ...
      for (const key in value) {
        try {
          if (convVal) {
            convVal[key] = (keySig as any).parse((value as any)[key], {
              suppressPathErrMsg: true,
            });
          } else {
            (keySig as any).parse((value as any)[key], {
              suppressPathErrMsg: true,
            });
          }
        } catch (err) {
      ...
      for (const key in value) {
        try {
          if (convVal) {
            // @ts-ignore
            convVal[key] = (
              this.objectShape[key] || keySig
            ).parse((value as any)[key], { suppressPathErrMsg: true });
          } else {
            // @ts-ignore
            (this.objectShape[key] || keySig).parse((value as any)[key], {
              suppressPathErrMsg: true,
            });
          }
        } catch (err) {
...
export class EnumType<T> extends Type<ValueOf<T>>
  implements Defaultable<ValueOf<T>> {
  private values: any[];
