...
export const api: z.Api<typeof router> = {
  ...
  "GET_PROJECTS": () =>
    app.services.projects.getAll().then((it) => ({ status: 200, content: it })),
  "CREATE_PROJECT": ({ body }) =>
    app.services.projects.create(body).then((_) => ({ status: 201 })),
  "UPDATE_PROJECT": ({ path, body }) =>
    app.services.projects.update(path[2], body).then((_) => ({ status: 201 })),
  "DELETE_PROJECT": ({ path }) =>
