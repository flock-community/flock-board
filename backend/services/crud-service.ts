...
export abstract class AbstractCrudService<E extends Entity>
  ...
  async update(id: E["id"], mutation: Partial<E>): Promise<E> {
    ...
    if (entityToUpdate === undefined) {
      throw new EntityNotFoundException(this.name, id);
    }
    const updatedEntity = { ...entityToUpdate, ...mutation };
    await this.updateAll(
      oldEntities.map((todo) => (todo.id === id ? updatedEntity : todo)),
    );
